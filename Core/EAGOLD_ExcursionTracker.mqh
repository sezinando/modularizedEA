...
void EAGOLD_ExcursionEnsureFileHeader(int handle){
   ...
   FileWrite(handle,"CYCLE_ID","START_TIME","END_TIME","DIRECTION","BUY_LOTS_MAX","SELL_LOTS_MAX","GROSS_LOTS_MAX","NET_LOTS_MAX","MFE","MAE","PEAK_FLOATING_PL","FINAL_REALIZED_PL","GIVEBACK","DURATION_SEC","REALIZATION_ENGINE","REALIZATION_TYPE");
}
...
void EAGOLD_ExcursionEnsureEventFileHeader(int handle){
   ...
   FileWrite(handle,
      ...
      "R12_VALID","R12_REGIME","R12_TIME","R12_CLOSE","R12_ATR","R12_ATR_RATIO","R12_DRIFT","R12_SLOPE_FAST","R12_SLOPE_SLOW","R12_RANGE_RATIO","R12_BODY_RATIO");
}
...
void EAGOLD_ExcursionWriteRecord(datetime endTime){
   ...
   double giveback=g_excursionMFE-g_excursionLastFloatingPL;if(giveback<0.0)giveback=0.0;
   ...
   FileWrite(handle,g_excursionCycleId,TimeToString(g_excursionStartTime,TIME_DATE|TIME_SECONDS),TimeToString(endTime,TIME_DATE|TIME_SECONDS),g_excursionDirection,
      DoubleToString(g_excursionMaxBuyLots,DigitsLots),DoubleToString(g_excursionMaxSellLots,DigitsLots),DoubleToString(g_excursionMaxGrossLots,DigitsLots),DoubleToString(g_excursionMaxNetLots,DigitsLots),
      DoubleToString(g_excursionMFE,2),DoubleToString(g_excursionMAE,2),DoubleToString(g_excursionPeakFloatingPL,2),DoubleToString(finalRealized,2),DoubleToString(giveback,2),IntegerToString((int)duration),g_excursionRealizationEngine,g_excursionRealizationType);
...
void EAGOLD_ExcursionWriteRealizationEvent(string engine,string action,EAGOLD_ActionResult result){
   ...
   FileSeek(handle,0,SEEK_END);EAGOLD_ExcursionEnsureEventFileHeader(handle);
   ...
   string r12Time=(g_r12State.valid?TimeToString(g_r12State.time,TIME_DATE|TIME_SECONDS):"");
...
void EAGOLD_ExcursionTrackerObserve(){
   ...
   if(masterOrders<=0){if(g_excursionActive)EAGOLD_ExcursionTrackerFinalize();return;}
...
void EAGOLD_ExcursionTrackerFinalize(){
   if(!g_excursionActive)return;datetime endTime=TimeCurrent();EAGOLD_ExcursionWriteRecord(endTime);double finalRealized=EAGOLDAccumulatedProfit()-g_excursionStartRealized;
