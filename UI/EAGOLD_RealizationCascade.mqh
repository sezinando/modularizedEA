...
void EAGOLD_ActionCascadeAdd(string engine)
{
   ...
   // Realization functions execute immediately before their corresponding
   // engine action marker. Bind that newest realization to the same engine
   // without changing the established lifecycle/R10 call contracts.
   if(g_realizationCascadeColorPending&&g_realizationCascadeCount>0)
   {
      g_realizationCascadeColors[0]=c;
      g_realizationCascadeColorPending=false;
   }
}
