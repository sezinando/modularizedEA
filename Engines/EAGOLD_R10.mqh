...
EAGOLD_ActionResult R10ProfitFundedAverageAdjustmentTransactional(int targetDirection,double availableCapital,double maxLots,double &usedCapital,double &reducedLots,double &realizedLoss)
{
   ...
   if(availableCapital<=0.0||maxLots<Lot)return(EAGOLD_ACTION_BLOCKED);
   int type=(targetDirection==OP_BUY?OP_BUY:OP_SELL);
   int ticket=-1;double ticketLots=0.0;double worstLoss=0.0;double worstOpen=0.0;
   for(int i=OrdersTotal()-1;i>=0;i--)
   {
      ...
      double money=OrderProfit()+OrderSwap()+OrderCommission();
      if(money>=0.0)continue;
      double open=OrderOpenPrice();
      bool moreAdverse=(ticket<0||(type==OP_BUY&&open>worstOpen)||(type==OP_SELL&&open<worstOpen));
      if(moreAdverse){ticket=OrderTicket();ticketLots=lots;worstLoss=money;worstOpen=open;}
   }
   if(ticket<0||worstLoss>=0.0)return(EAGOLD_ACTION_BLOCKED);
   double lossPerLot=MathAbs(worstLoss)/ticketLots;
   if(lossPerLot<=0.0)return(EAGOLD_ACTION_BLOCKED);
   double closeLots=MathMin(ticketLots,maxLots);
   closeLots=MathMin(closeLots,availableCapital/lossPerLot);
   double lotStep=MarketInfo(Symbol(),MODE_LOTSTEP);
   double minLot=MarketInfo(Symbol(),MODE_MINLOT);
   if(lotStep<=0.0)lotStep=minLot;
   if(lotStep<=0.0)lotStep=Lot;
   closeLots=MathFloor((closeLots+1.0e-10)/lotStep)*lotStep;
   closeLots=NormalizeDouble(closeLots,DigitsLots);
   if(closeLots<minLot||closeLots<Lot)return(EAGOLD_ACTION_BLOCKED);
   ...
   if(usedCapital>availableCapital+0.01)
   {
      ...
      CreateR10VisualMarker(targetDirection,closeLots,money,beforeExposure,afterExposure,beforeGross,afterGross);
      CreateEngineActionMarker("R10","AVG_OVERSHOOT",targetDirection,closeLots);
      return(EAGOLD_ACTION_PARTIAL);
   }

   CreateR10VisualMarker(targetDirection,closeLots,money,beforeExposure,afterExposure,beforeGross,afterGross);
   CreateEngineActionMarker("R10","AVG_ADJUST",targetDirection,closeLots);
...
bool R10ProfitFundedAverageAdjustment(int targetDirection,double availableCapital,double maxLots,double &usedCapital,double &reducedLots,double &realizedLoss)
{
   ...
   return(result==EAGOLD_ACTION_COMPLETED);
}
