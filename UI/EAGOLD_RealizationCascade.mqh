...
color CascadeEngineColor(string engine){if(engine=="R1")return(clrYellow);if(engine=="R1.1")return(clrBlue);if(engine=="R4")return(clrRed);if(engine=="R5")return(clrMagenta);if(engine=="BRX")return(clrAqua);if(engine=="R7")return(clrOrange);if(engine=="R10")return(clrViolet);if(engine=="R11")return(clrAqua);if(engine=="R13")return(clrLime);return(clrWhite);}
...
void EAGOLD_RealizationCascadeUpdate()
{
   ...
   ObjectSetInteger(0,actionTitle,OBJPROP_CORNER,CORNER_RIGHT_UPPER);ObjectSetInteger(0,actionTitle,OBJPROP_ANCHOR,ANCHOR_RIGHT_UPPER);ObjectSetInteger(0,actionTitle,OBJPROP_XDISTANCE,actionRight+7);ObjectSetInteger(0,actionTitle,OBJPROP_YDISTANCE,panelY+8);ObjectSetInteger(0,actionTitle,OBJPROP_SELECTABLE,false);ObjectSetInteger(0,actionTitle,OBJPROP_HIDDEN,true);ObjectSetString(0,actionTitle,OBJPROP_FONT,"Arial Bold");ObjectSetInteger(0,actionTitle,OBJPROP_FONTSIZE,9);ObjectSetInteger(0,actionTitle,OBJPROP_COLOR,clrWhite);ObjectSetInteger(0,actionTitle,OBJPROP_ZORDER,1101);ObjectSetString(0,actionTitle,OBJPROP_TEXT,"ACOES DO EA (FIFO - 30)");
