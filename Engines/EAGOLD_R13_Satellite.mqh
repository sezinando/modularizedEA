...
EAGOLD_ActionResult R13TryOpenTransactional(int masterDirection,double masterExposure,int regime,double &openedLots){
   openedLots=0.0;
   if(!EnableR13Trading)return(EAGOLD_ACTION_BLOCKED);
   if(masterDirection<0||masterExposure<R13MinDirectionalImbalance)return(EAGOLD_ACTION_BLOCKED);
   if(R13CountOwnPositions()>0)return(EAGOLD_ACTION_BLOCKED);
   if(R13EntryCooldownSeconds>0.0&&g_r13LastEntry>0&&TimeCurrent()-g_r13LastEntry<R13EntryCooldownSeconds)return(EAGOLD_ACTION_BLOCKED);
   int direction=R13ComplementaryDirection(masterDirection);
   if(direction<0)return(EAGOLD_ACTION_BLOCKED);
   double room=R13MaxLots-R13OwnLots();
   if(room<Lot)return(EAGOLD_ACTION_BLOCKED);
   double lot=NormalizeDouble(MathMin(Lot,room),DigitsLots);
   double minLot=MarketInfo(Symbol(),MODE_MINLOT);
   if(lot<minLot||lot<Lot)return(EAGOLD_ACTION_BLOCKED);
   if(!R13RiskGate(direction,lot))return(EAGOLD_ACTION_BLOCKED);
   int ticket=R13SendMarket(direction,lot);
   if(ticket<=0){Print(EA_NAME," R13 ENTRY TRANSACTION FAILED: side=",(direction==OP_BUY?"BUY":"SELL")," lots=",DoubleToString(lot,DigitsLots)," regime=",R13RegimeName(regime));return(EAGOLD_ACTION_FAILED);}
   g_r13LastEntry=TimeCurrent();
   openedLots=lot;
   Print(EA_NAME," R13 ENTRY TRANSACTION COMPLETED: ticket=",ticket," regime=",R13RegimeName(regime));
