...
#include "../Core/EAGOLD_ExposureReliefTelemetry.mqh"
...
double R13TodayClosedNetProfit(){datetime now=TimeCurrent();datetime dayStart=StringToTime(TimeToString(now,TIME_DATE));double total=0.0;for(int i=OrdersHistoryTotal()-1;i>=0;i--){if(!OrderSelect(i,SELECT_BY_POS,MODE_HISTORY))continue;if(!IsR13Order())continue;int type=OrderType();if(type!=OP_BUY&&type!=OP_SELL)continue;if(OrderCloseTime()<dayStart)continue;total+=OrderProfit()+OrderSwap()+OrderCommission();}return(total);}

void R13ResetObserverState(R13ObserverState &state){state.configValid=false;state.enabled=false;state.tradingEnabled=false;state.eligible=false;state.regime=R13_REGIME_OFF;state.masterDirection=-1;state.satelliteDirection=-1;state.atrPoints=0.0;state.rangePoints=0.0;state.relativeVolatility=0.0;state.driftPoints=0.0;state.masterExposureLots=0.0;state.masterDrawdown=0.0;state.satelliteLots=0.0;state.satelliteProfit=0.0;state.recoveryCapital=g_r13RecoveryCapitalAvailable;state.recoveryCapitalUsed=g_r13RecoveryCapitalUsed;state.satellitePositions=0;state.reason="OFF";}
...
int R13SendMarket(int type,double lots){int ticket=SendMarketByMagic(type,lots,R13OrderComment,R13MagicNumber);if(ticket>0)Print(EA_NAME," R13 ENTRY: ticket=",ticket," side=",(type==OP_BUY?"BUY":"SELL")," lots=",DoubleToString(lots,DigitsLots)," masterExposure=",DoubleToString(ExposureLots(),DigitsLots));return(ticket);}
...
EAGOLD_ActionResult R13CloseAllTransactional(string reason,int &requestedCount,int &completedCount,double &requestedLots,double &completedLots,double &realizedTotal){requestedCount=0;completedCount=0;requestedLots=0.0;completedLots=0.0;realizedTotal=0.0;int tickets[];ArrayResize(tickets,0);for(int i=OrdersTotal()-1;i>=0;i--){if(!OrderSelect(i,SELECT_BY_POS,MODE_TRADES))continue;if(!IsR13Order())continue;int type=OrderType();if(type!=OP_BUY&&type!=OP_SELL)continue;int n=ArraySize(tickets);ArrayResize(tickets,n+1);tickets[n]=OrderTicket();requestedCount++;requestedLots+=OrderLots();}if(requestedCount<=0)return(EAGOLD_ACTION_BLOCKED);for(int j=0;j<ArraySize(tickets);j++){int ticket=tickets[j];double beforeLots=0.0;if(OrderSelect(ticket,SELECT_BY_TICKET,MODE_TRADES))beforeLots=OrderLots();double realized=0.0;if(R13CloseTicket(ticket,realized)){completedCount++;completedLots+=beforeLots;realizedTotal+=realized;}}int remaining=R13CountOwnPositions();if(completedCount==requestedCount&&remaining==0){Print(EA_NAME," R13 TRANSACTION COMPLETED: reason=",reason," positions=",completedCount," lots=",DoubleToString(completedLots,DigitsLots)," realized=$",DoubleToString(realizedTotal,2));g_r13LastEntry=TimeCurrent();return(EAGOLD_ACTION_COMPLETED);}if(completedCount>0){Print(EA_NAME," R13 TRANSACTION PARTIAL: reason=",reason," requested=",requestedCount," completed=",completedCount," requestedLots=",DoubleToString(requestedLots,DigitsLots)," completedLots=",DoubleToString(completedLots,DigitsLots)," remaining=",remaining," realized=$",DoubleToString(realizedTotal,2));return(EAGOLD_ACTION_PARTIAL);}Print(EA_NAME," R13 TRANSACTION FAILED: reason=",reason," requested=",requestedCount," remaining=",remaining);return(EAGOLD_ACTION_FAILED);}

void R13AddRecoveryCapital(double realized){if(realized<=0.0||R13RecoveryCapitalFraction<=0.0)return;double capitalBefore=g_r13RecoveryCapitalAvailable;double added=realized*R13RecoveryCapitalFraction;g_r13RecoveryCapitalAvailable+=added;EAGOLD_ExposureReliefTelemetryWriteCapitalCredit(realized,added,capitalBefore,g_r13RecoveryCapitalAvailable,R13OwnDirection());Print(EA_NAME," R13 RECOVERY CAPITAL: realized=$",DoubleToString(realized,2)," added=$",DoubleToString(added,2)," available=$",DoubleToString(g_r13RecoveryCapitalAvailable,2));}
...
EAGOLD_ActionResult R13TryFundMasterAdjustmentTransactional(double &usedCapital,double &reducedLots,double &realizedLoss){
   ...
   double capitalBefore=g_r13RecoveryCapitalAvailable;
   double grossBefore=DirectionLots(OP_BUY)+DirectionLots(OP_SELL);
   double targetLotsBefore=DirectionLots(masterDirection);
   int targetCountBefore=EAGOLD_ExposureReliefOpenCount(masterDirection);
   double avgBefore=EAGOLD_ExposureReliefWeightedAverage(masterDirection);
   double targetFloatingBefore=EAGOLD_ExposureReliefDirectionFloatingProfit(masterDirection);

   EAGOLD_ActionResult result=R10ProfitFundedAverageAdjustmentTransactional(masterDirection,g_r13RecoveryCapitalAvailable,R13MasterAdjustmentMaxLots,usedCapital,reducedLots,realizedLoss);
   double exposureAfter=ExposureLots();
   EAGOLD_ExposureReliefTelemetryWriteAdjustment(result,masterDirection,capitalBefore,usedCapital,reducedLots,realizedLoss,exposureBefore,grossBefore,targetLotsBefore,targetCountBefore,avgBefore,targetFloatingBefore);
   ...
   if(result==EAGOLD_ACTION_COMPLETED){
      if(reducedLots<Lot||exposureAfter>=exposureBefore-0.00001){
         ...
         if(exposureAfter<exposureBefore-0.00001){if(usedCapital>0.0){g_r13RecoveryCapitalAvailable=MathMax(0.0,g_r13RecoveryCapitalAvailable-usedCapital);g_r13RecoveryCapitalUsed+=usedCapital;}return(EAGOLD_ACTION_PARTIAL);}return(EAGOLD_ACTION_FAILED);
      }
      g_r13RecoveryCapitalAvailable=MathMax(0.0,g_r13RecoveryCapitalAvailable-usedCapital);g_r13RecoveryCapitalUsed+=usedCapital;
   ...
   if(result==EAGOLD_ACTION_PARTIAL){if(usedCapital>0.0){g_r13RecoveryCapitalAvailable=MathMax(0.0,g_r13RecoveryCapitalAvailable-usedCapital);g_r13RecoveryCapitalUsed+=usedCapital;}Print(EA_NAME," R13 -> R10 TRANSACTION PARTIAL: reduced=",DoubleToString(reducedLots,DigitsLots)," loss=$",DoubleToString(MathAbs(realizedLoss),2)," used=$",DoubleToString(usedCapital,2)," exposureBefore=",DoubleToString(exposureBefore,DigitsLots)," exposureAfter=",DoubleToString(exposureAfter,DigitsLots));return(EAGOLD_ACTION_PARTIAL);}
   if(result==EAGOLD_ACTION_FAILED){Print(EA_NAME," R13 -> R10 TRANSACTION FAILED: exposureBefore=",DoubleToString(exposureBefore,DigitsLots)," exposureAfter=",DoubleToString(exposureAfter,DigitsLots));return(EAGOLD_ACTION_FAILED);}
   return(result);
}
...
EAGOLD_ActionResult R13TryOpenTransactional(int masterDirection,double masterExposure,int regime,double &openedLots){openedLots=0.0;if(!EnableR13Trading)return(EAGOLD_ACTION_BLOCKED);if(masterDirection<0||masterExposure<R13MinDirectionalImbalance)return(EAGOLD_ACTION_BLOCKED);if(R13CountOwnPositions()>0)return(EAGOLD_ACTION_BLOCKED);if(R13EntryCooldownSeconds>0.0&&g_r13LastEntry>0&&TimeCurrent()-g_r13LastEntry<R13EntryCooldownSeconds)return(EAGOLD_ACTION_BLOCKED);int direction=R13ComplementaryDirection(masterDirection);if(direction<0)return(EAGOLD_ACTION_BLOCKED);double room=R13MaxLots-R13OwnLots();if(room<Lot)return(EAGOLD_ACTION_BLOCKED);double lot=NormalizeDouble(MathMin(Lot,room),DigitsLots);double minLot=MarketInfo(Symbol(),MODE_MINLOT);if(lot<minLot||lot<Lot)return(EAGOLD_ACTION_BLOCKED);if(!R13RiskGate(direction,lot))return(EAGOLD_ACTION_BLOCKED);int ticket=R13SendMarket(direction,lot);if(ticket<=0){Print(EA_NAME," R13 ENTRY TRANSACTION FAILED: side=",(direction==OP_BUY?"BUY":"SELL")," lots=",DoubleToString(lot,DigitsLots)," regime=",R13RegimeName(regime));return(EAGOLD_ACTION_FAILED);}g_r13LastEntry=TimeCurrent();openedLots=lot;Print(EA_NAME," R13 ENTRY TRANSACTION COMPLETED: ticket=",ticket," regime=",R13RegimeName(regime));return(EAGOLD_ACTION_COMPLETED);}

void R13Observe(R13ObserverState &state){R13ResetObserverState(state);state.configValid=IsR13OwnershipConfigurationValid();state.enabled=EnableR13;state.tradingEnabled=(EnableR13&&EnableR13Trading&&EnableR13AutoActivation);state.masterExposureLots=ExposureLots();state.masterDirection=HeavyDirection();state.satelliteDirection=R13OwnDirection();state.satelliteLots=R13OwnLots();state.satelliteProfit=R13OwnProfit();state.satellitePositions=R13CountOwnPositions();state.recoveryCapital=g_r13RecoveryCapitalAvailable;state.recoveryCapitalUsed=g_r13RecoveryCapitalUsed;if(!state.configValid){state.reason="INVALID_OWNERSHIP_CONFIG";return;}if(!state.enabled){state.reason="DISABLED";return;}int period=14;state.atrPoints=R13AverageTrueRangePoints(period);state.rangePoints=R13WindowRangePoints(period);state.driftPoints=R13WindowDriftPoints(period);state.relativeVolatility=(state.atrPoints>0.0?state.rangePoints/state.atrPoints:0.0);state.regime=R13ClassifyRegime(state.atrPoints,state.rangePoints,state.driftPoints);state.eligible=(state.masterExposureLots>=R13MinDirectionalImbalance&&state.configValid&&state.satellitePositions<R13MaxPositions);state.reason=state.eligible?"ELIGIBLE_MASTER_EXPOSURE":"MASTER_OR_LIMIT_GATE";if(state.tradingEnabled){double usedCapital=0.0,reducedLots=0.0,realizedLoss=0.0;EAGOLD_ActionResult funding=R13TryFundMasterAdjustmentTransactional(usedCapital,reducedLots,realizedLoss);if(funding!=EAGOLD_ACTION_BLOCKED){EAGOLD_ApplyActionResult(funding,"R13","R10_MASTER_ADJUST",state.masterDirection,reducedLots);if(funding!=EAGOLD_ACTION_COMPLETED)return;return;}R13ManageOpenPositions(state.masterDirection,state.masterExposureLots);if(!EAGOLD_EconomicExecutionAllowed())return;double openedLots=0.0;EAGOLD_ActionResult entry=R13TryOpenTransactional(state.masterDirection,state.masterExposureLots,state.regime,openedLots);if(entry!=EAGOLD_ACTION_BLOCKED)EAGOLD_ApplyActionResult(entry,"R13","OPEN",R13ComplementaryDirection(state.masterDirection),openedLots);state.masterExposureLots=ExposureLots();state.masterDirection=HeavyDirection();state.satelliteDirection=R13OwnDirection();state.satelliteLots=R13OwnLots();state.satelliteProfit=R13OwnProfit();state.satellitePositions=R13CountOwnPositions();state.recoveryCapital=g_r13RecoveryCapitalAvailable;state.recoveryCapitalUsed=g_r13RecoveryCapitalUsed;}}
